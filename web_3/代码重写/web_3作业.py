...
class User(Model):
    ...
    def validate_login(self):
        test = []
        users = load('db/user.txt')
        for user in users:
            test.append(self.username in user.get('username', '')
...
def parsed_header(request):
    """
    输入 string 格式的 request，形如：
    GET / HTTP/1.1
    Host: localhost:3000
    ...
    header_others = header.split('\r\n')[1:]
    for i in header_others:
        # print(i)
        k, v = i.split(': ', 1)
        header_dict[k] = v
    # print(header_dict)
...
# http: // www.ituring.com.cn / book / tupubarticle / 1204


# parsed_header("GET / HTTP/1.1\r\nHost: localhost:3000\r\nConnection: keep-alive\r\nUpgrade-Insecure-Requests: 1\r\nUser-Agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.169 Safari/537.36\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3\r\nAccept-Encoding: gzip, deflate, br\r\nAccept-Language: en-US,en;q=0.9,zh-CN;q=0.8,zh;q=0.7\r\n\r\n")
